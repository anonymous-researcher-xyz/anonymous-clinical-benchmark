"""
Zero-Shot Inference Script: Evaluates LLMs on benchmark datasets using zeroshot without fine-tuning.
Supports extended reasoning mode for compatible models.
"""
...
BATCH_SIZE = 8
MAX_TOKENS = 512
MAX_TOKENS_EXTENDED = 1024  # For thinking mode and summarization
...
def add_thinking_instruction(prompt: str, use_thinking: bool = False) -> str:
    """Add thinking instruction to any prompt if thinking mode is enabled."""
    if use_thinking:
        thinking_instruction = (
            "\n\nAntes de fornecer a resposta final, pense passo a passo de forma concisa, "
            "limitando o raciocínio (thinking) a no máximo 80 palavras."
        )
        return prompt + thinking_instruction
    return prompt

def generate_prompt_drbodebench(use_thinking: bool = False) -> str:
    ...
    base_prompt = (
        ...
        "Resposta: <LETRA>.\n"
        "Breve justificativa: <após informar a letra, dê uma breve justificativa da escolha baseada no conteúdo da questão, sem adicionar suposições>"
    )
    
    return add_thinking_instruction(base_prompt, use_thinking)

def generate_prompt_clinical_ner(use_thinking: bool = False) -> str:
...
def generate_prompt_semclinbr(use_thinking: bool = False) -> str:
...
def format_prompt(example: Dict, dataset_name: str, use_thinking: bool = False) -> Tuple[List[Dict], str, Any, str]:
    """
    ...
    if dataset_name == "amazon_diseases":
        ...
        system_prompt = add_thinking_instruction(system_prompt, use_thinking)
        ...
        system_prompt = generate_prompt_drbodebench(use_thinking)
        ...
        system_prompt = generate_prompt_clinical_ner(use_thinking)
        text = example["text"]
        messages = [
            {"role": "system", "content": system_prompt},
        ...
        system_prompt = generate_prompt_semclinbr(use_thinking)
        text = example["text"]
        messages = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": text}
...
def extract_thinking_content(text: str) -> Tuple[str, str]:
    ...
    Returns: (clean_response, thinking_content)
    """
...
def run_batch_inference(
    ...
    logger.info(f"Thinking mode: {'ENABLED' if use_thinking else 'DISABLED'}")
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        for start_idx in tqdm(range(0, len(dataset), batch_size), desc="Processing batches"):
            ...
            for item in batch_data:
                try:
                    messages, item_id, reference, text = format_prompt(item, dataset_name, use_thinking)
                    ...
                    try:
                        ...
                        # Fallback 1: if enable_thinking is not supported
                        try:
                            chat_formatted_prompt = tokenizer.apply_chat_template(
                                messages,
                                tokenize=False,
            ...
            try:
                ...
                for i, generated_text in enumerate(generated_texts):
                    clean_response, thinking_content = extract_thinking_content(generated_text.strip())
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Zero-shot evaluation of LLMs on clinical benchmark with thinking mode support'
    )
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Enable thinking mode for extended reasoning (uses more tokens)'
    )
    ...
    # Adjust max tokens if thinking mode is enabled
    max_tokens = MAX_TOKENS_EXTENDED if args.thinking else args.max_tokens

    safe_model_name = model_name.replace("/", "_")
    
    # Generate output path with thinking suffix if enabled
    suffix = "_thinking" if args.thinking else ""
    default_output = (
        Path("../../results/predictions")
        / f"{args.dataset_name}_{safe_model_name}_zeroshot{suffix}_predictions.jsonl"
    )
